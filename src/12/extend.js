...
function link(source) {
  if (source && source.uniforms) {
    for (let name in source.uniforms) {
      if (source.uniforms[name].linked) this.uniforms[name] = source.uniforms[name];
    }
  }
...
function copy(source) {
  ...
  if (source.customDepthMaterial) this.customDepthMaterial = source.customDepthMaterial.clone().link(this);

  if (source.customDistanceMaterial) this.customDistanceMaterial = source.customDistanceMaterial.clone().link(this);
...
function extend(source, object) {
  ...
  const MaterialClass =
    object.class || (source.isMaterial && source.constructor ? source.constructor : null) || CustomMaterial;
  ...
  if (source.templates instanceof Array) material.templates = source.templates.concat(material.templates);
  ...
  if (object.vertex !== undefined) vertexShader = applyPatches(vertexShader, object.vertex);

  if (object.fragment !== undefined) fragmentShader = applyPatches(fragmentShader, object.fragment);
  ...
  if (base && base.templates && base.templates.length) {
    ...
    for (let template of base.templates) patchShader(properties, template, mixUniforms, defines);
  ...
  if (material.isCustomMaterial) {
    for (let name in uniforms) {
      if (mapFlags[name] && uniforms[name].value) material[name] = uniforms[name].value;
    }
  }
    ...
    customDepthMaterial: {
      get: function () {
        return (
          ...
          (this.material && this.material.customDepthMaterial ? this.material.customDepthMaterial : undefined)
        );
      },
    ...
    customDistanceMaterial: {
      get: function () {
        return (
          ...
          (this.material && this.material.customDistanceMaterial ? this.material.customDistanceMaterial : undefined)
        );
      },
...
Object.defineProperties(extend.CustomMaterial.prototype, {
  ...
  specular: {
    get: function () {
      if (this.uniforms.specular === undefined) this.uniforms.specular = { value: new THREE.Color('white') };

      return this.uniforms.specular.value;
    },

    set: function (value) {
      if (this.uniforms.specular === undefined) this.uniforms.specular = { value };
  ...
  shininess: {
    ...
    set: function (value) {
      if (this.uniforms.shininess === undefined) this.uniforms.shininess = { value };
...
function useUniformPairs(instance, uniforms) {
  ...
  if (instance.displacementMap) cloneUniforms(UniformsLib.displacementmap, uniforms);

  if (instance.clearcoatNormalMap) cloneUniforms(UniformsLib.clearcoatnormalmap, uniforms);
}
...
function useUniforms(name, object, uniforms) {
  ...
  if (requiredUniforms[name] !== undefined) cloneUniforms(requiredUniforms[name], uniforms);
  ...
  if (lights) {
    if (object.use) {
      ...
      if (use.indexOf('LightProbe') > -1) cloneUniforms(UniformsLib.lights.lightProbe, uniforms);

      if (use.indexOf('AmbientLight') > -1) cloneUniforms(UniformsLib.lights.ambientLightColor, uniforms);

      if (use.indexOf('ReactAreaLight') > -1) cloneUniforms(UniformsLib.lights.rectAreaLights, uniforms);

      if (use.indexOf('HemisphereLight') > -1) cloneUniforms(UniformsLib.lights.hemisphereLights, uniforms);
    } else {
...
function applyConstants(name, uniform, defines, object, instance) {
  ...
  if (uniform && uniform.value && localsMapping[name] !== undefined) instance[name] = uniform.value;
  ...
  if (flag !== undefined && (flag.not === undefined || flag.not !== value)) defines[flag.as] = uniform.value;
}
...
function patchShader(shader, object, uniformsMixer = applyUniforms, defines = null) {
  ...
  if (object.vertexEnd) vertexShader = vertexShader.replace(/\}(?=[^.]*$)/g, object.vertexEnd + '\n}');
  ...
  if (uniformsMixer instanceof Function) uniformsMixer(object, shader, defines);
